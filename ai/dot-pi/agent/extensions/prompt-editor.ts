...
// Avoid editor takeovers and startup layout shifts. Keep mode commands/shortcuts,
// but let other extensions (e.g. atelier) own editor chrome.
const ENABLE_PROMPT_EDITOR_COMPONENT = false;
...
function applyEditor(pi: ExtensionAPI, ctx: ExtensionContext) {
	if (!ctx.hasUI) return;
	if (!ENABLE_PROMPT_EDITOR_COMPONENT) return;
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (!ENABLE_PROMPT_EDITOR_COMPONENT) {
	...
	pi.on("model_select", async (event: ModelSelectEvent, ctx) => {
		...
		if (ctx.hasUI && ENABLE_PROMPT_EDITOR_COMPONENT) {
			requestEditorRender?.();
		}
	});
