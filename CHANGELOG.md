...
### Fixed
- Stopped stowing `ai/dot-codex/config.toml`. Codex rewrites that file in place on every run, stamping absolute paths (`CODEX_HOME`, bundled-plugin paths under the running user's home) into `mcp_servers` and appending a `[projects."..."]` trust entry per directory visited. Sharing it by symlink published one machine's home directory to every other machine — a second Mac got a `node_repl` pointing at a home that does not exist there, so the browser and computer-use plugins could not start — and produced hundreds of lines of trust churn in every diff. An `ai/.stow-local-ignore` excludes just that file; it stays tracked as the reference copy of the preferences worth sharing. `make setup` reads the same ignore list so it no longer moves the machine's own copy aside.
